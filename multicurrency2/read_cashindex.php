...
//$filecontent = file_get_contents("../bloomberg-input2/cashindex_sl.csv.".date("Ymd", strtotime($date)));
$filecontent= file_get_contents("../files/input/cashindex-header.csv.". date("Ymd", strtotime($date)));
...
if($filecontent)
{
	...
	if(!empty($csvdata))
	{
		...
		{	
			...
			$data['isin']="'".$security[0]."'";
			$data['price']="'".$security[3]."'";
			$data['date']="'".$date."'";
	
			if(is_numeric($security[3]))
			{
				qry_insert('tbl_cash_prices',$data);
				$i++;
			}
			else
			{
...
else
{
	...
	mail("dbajpai@indxx.com","Softlayer - File Read Error!","cashprices.csv for today is not available - ".date("Y-m-d"));
	//exit;"Y"
}
