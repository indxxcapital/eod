...
$Query=mysql_query("Select  * from tbl_indxx  where  1=1 ");
...
if(mysql_num_rows($Query)>0)
{
	while($index=mysql_fetch_assoc($Query))
	{
		...
		if($convert_flag  && $index['status'] && $index['usersignoff'] && $index['dbusersignoff'] && $index['submitted'])
		{
	 		...
	 		$query="SELECT it.isin,it.ticker, pf.price as  localprice ,pf.curr as local_currency,it.curr as ticker_currency FROM `tbl_indxx_ticker` it
