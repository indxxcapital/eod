...
$emailsids='';
$dbuseremailsids='';
$indxxs=selectrow(array('id'),'tbl_indxx_temp');
...
if($convert_flag  && $index['status'] && $index['submitted'] )
{
...
// $query="SELECT it.isin,it.ticker,(select price from tbl_prices_local_curr pf where pf.isin=it.isin  and pf.date='".$date."') as localprice ,(select curr from tbl_prices_local_curr pf where pf.isin=it.isin  and pf.date='".$date."') as local_currency,it.curr as ticker_currency FROM `tbl_indxx_ticker_temp` it where it.indxx_id='".$index['id']."'";
 ...
 $query="SELECT it.isin,it.ticker, pf.price as  localprice ,pf.curr as local_currency,it.curr as ticker_currency FROM `tbl_indxx_ticker_temp` it
 left join tbl_prices_local_curr pf on pf.isin=it.isin 
  where it.indxx_id='".$index['id']."' and pf.date='".$date."'";
$res=mysql_query($query);
if(mysql_num_rows($res)>0)
{$i=0;
...
foreach($indexarray as $keyindex=>$valueindex)
{
	...
	while($users=mysql_fetch_assoc($useremailres))
	{
		...
		if(!empty($emailsids))	
		{
			...
			$sub='Softlayer - ICAI Currency Mismatch Notification';
	...
	$dbuseremailsids=substr($dbuseremailsids,0, -1);
		...
		//echo "update tbl_indxx_temp set status='0' where id='".$keyindex."'";
