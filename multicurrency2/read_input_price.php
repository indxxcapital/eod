...
//$filecontent=file_get_contents("../bloomberg-input2/multicurr_sl.csv.".date("Ymd",strtotime($date)));
$filecontent= file_get_contents("../files/input/multicurr-header.csv.". date("Ymd", strtotime($date)));
...
if($filecontent)
{
	...
	while($start<$limit+($_GET['id']) && $start<$pagecount)
	{
		...
		{	
			...
			mail("dbajpai@indxx.com","Softlayer - Non Numeric Price in price file!","Price is not a number for ".$security[0]);		
		}	
		$start++;
	}
