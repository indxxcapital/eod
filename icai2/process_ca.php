...
if ($counter * $perReq >= $total) {
	saveProcess ();
	// echo "Completed";
	echo '<script>document.location.href="checkcurrency.php";</script>';
} else {
	...
	if (mysql_num_rows ( $res ) > 0) {
		...
		while ( $row = mysql_fetch_assoc ( $res ) ) {
			...
			$ca_field_id = selectrow ( array (
					'id' 
			), 'tbl_ca_subcategory', array (
					'code' => $security ['5'] 
			) );
			...
			if ($checkArray ['mnemonic'] == '' || $checkArray ['ann_date'] == '0000-00-00' || $checkArray ['eff_date'] == '0000-00-00') {
				...
				for($k = 1; $k < ($num_fields * 2) + 1; $k = $k + 2) {
					// echo ($k+20)."<br>";
					$field_id = selectrow ( array (
							'id' 
					), 'tbl_ca_action_fields', array (
							'field_name' => $security [$k + 20] 
					) );
					$data2 ['ca_id'] = "'" . $ca_id . "'";
	...
	echo '<script>document.location.href="process_ca.php?counter=' . $counter . '";</script>';
}
