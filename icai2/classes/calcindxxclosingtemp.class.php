...
class Calcindxxclosingtemp extends Application{
	...
	{		
		...
		//exit;
		$indxxs=$this->db->getResult("select tbl_indxx_temp.* from tbl_indxx_temp  where status='1' and usersignoff='1' and dbusersignoff='1'  and submitted='1' limit $page,1",true);
		
		$totalindxxs=$this->db->getResult("select tbl_indxx_temp.id from tbl_indxx_temp  where status='1' and usersignoff='1' and dbusersignoff='1' and submitted='1'",true);
  ...
  ";			
	//exit;
		...
		//$this->pr($indxxprices,true);
			...
			$file="../files2/ca-output/".$client['ftpusername']."/Closing-".strtolower($row['code'])."p-".$datevalue.".txt";
...
if($row['divpvalue'])
{
 ...
 //	$newindexvalue=number_format(($marketValue/$newDivisor),2,'.','');
		...
		//exit;
	$insertQuery='INSERT into tbl_indxx_value_temp (indxx_id,code,market_value,indxx_value,date,olddivisor,newdivisor) values ("'.$row['id'].'","'.$row['code'].'","'.$marketValue.'","'.$newindexvalue.'","'.$datevalue.'","'.$oldDivisor.'","'.$newDivisor.'")';
