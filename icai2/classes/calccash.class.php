...
class Calccash extends Application{
	...
	//	echo "select price from tbl_cash_prices  where isin like '%".$row['isin']."%' order by date desc ";
		...
		{  file_put_contents('../files2/backup/preCLOSECASHdata'.date("Y-m-d-H-i-s").'.json', json_encode($final_array));
 ...
 if(   fwrite($open,$entry1.$entry2.$entry3.$entry4))
{
		 ...
		 file_put_contents('../files2/backup/postOPENCASHdata'.date("Y-m-d-H-i-s").'.json', json_encode($final_array));
		}
