...
class Calcspinstockadd extends Application 
{
	...
	function index() 
	{
			...
			foreach ( $finalArray as $ca ) 
			{
				if (! empty ( $ca )) 
				{
					foreach ( $ca ['newTickers'] as $newTicker ) 
					{
						...
						$insertPriceQuery = "Insert into tbl_final_price set dateAdded='" . $date . "',isin='" . $newTicker ['isin'] . "',date='" . $newTicker ['lastday'] . "',price='" . $newTicker ['lastdayvalue'] . "',currencyfactor='" . $newTicker ['lastdaycurrencyvalue'] . "', 	localprice='" . $newTicker ['lastdaylocalvalue'] . "', indxx_id='" . $newTicker ['indxx_id'] . "' ";
		...
		if (DEBUG)
		{
			$this->Redirect2("index.php?module=calcspinstockaddtemp&DEBUG=" .DEBUG. "&date=" .$date. "&log_file=" . basename(log_file), "", "" );
		}
		else
		{
			//$this->Redirect("index.php?module=calcspinstockaddtemp&DEBUG=" .DEBUG. "&date=" .$date. "&log_file=" . basename(log_file), "", "" );
			log_error("Unable to locate calcspinstockaddtemp index module.");
