...
{
$priority_array=$this->db->getResult("select distinct(priority) as priority from tbl_indxx union select distinct(priority) as priority from tbl_indxx_temp union select distinct(priority) as priority from tbl_indxx_cs union select distinct(priority) as priority from tbl_indxx_sl   ",true);
//$this->pr($priority_array,true);
$_SESSION['priority']=$priority_array;
asort($_SESSION['priority']);

$_SESSION['currentPriority']=$_SESSION['priority'][0];
$_SESSION['currentPriorityIndex']=0;
//print_r($_SESSION['currentPriority']);
}
else{
	if($_SESSION['priority'][$_SESSION['currentPriorityIndex']+1])
{	
$_SESSION['currentPriority']=$_SESSION['priority'][$_SESSION['currentPriorityIndex']+1];

$_SESSION['currentPriorityIndex']+=1;
//print_r($_SESSION['currentPriority']);
}
}
*///print_r($_SESSION['currentPriority']);
...
//$priority_array
	 ...
	 if($_GET['id'])
		...
		$limit=5;
//exit;
	$indexdata=$this->db->getResult("select tbl_indxx_temp.* from tbl_indxx_temp where 1=1 and status='1' and usersignoff='1' and dbusersignoff='1' and submitted='1' and finalsignoff='1'  and dateStart='".$this->_date."'  limit $page,1",true);
	
	$totalindxxs=$this->db->getResult("select tbl_indxx_temp.* from tbl_indxx_temp where 1=1 and status='1' and usersignoff='1' and dbusersignoff='1' and submitted='1' and finalsignoff='1'  and dateStart='".$this->_date."'  ",true);
...
$newReplaceId=mysql_insert_id();
	...
	if($totalindexes<=$page)
		...
		$this->saveProcess(1);
		$this->Redirect2("index.php?module=replacecash","","");			
		}
		else
		{
			$this->saveProcess(1);
			$this->Redirect2("index.php?module=replaceindex&event=index&id=".($page+1),"","");	
		}
