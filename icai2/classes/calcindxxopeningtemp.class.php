...
//	$priorityArray=max($_SESSION['priority']);
	...
	//	$this->pr($_SESSION,true);
		...
		// 
		$indxxs=$this->db->getResult("select tbl_indxx_temp.* from tbl_indxx_temp  where status='1' and usersignoff='1' and dbusersignoff='1' and submitted='1' and runindex='1' limit  $page,$limit ",true);	
...
if($type="open"){
		  file_put_contents('../files2/backup/preopentempdata'.date("Y-m-d-H-i-s").'.json', json_encode($final_array));
...
//$this->db->query('INSERT into tbl_dividend_ph (date,indxx_id, ticker_id,share,dividend) values ("'.$datevalue2.'","'.$closeIndxx['id'].'","'.$closeprices['id'].'","'.$closeprices['calcshare'].'","'.$ca_prices['ca_price_index_currency'].'")');
		...
		//			$final_array[$indxxKey]['values'][$securityKey]['newisin']=$newisin;
			 ...
			 $final_array[$indxxKey]['index_value']['divisor_impact']+=	$divisorImpact;;
			 	//echo "<br>";	
				  ...
				  file_put_contents('../files2/backup/postopentempdata'.date("Y-m-d-H-i-s").'.json', json_encode($final_array));
	...
	if(!empty($final_array))
	{
		...
		{
			$file="../files2/ca-output_upcomming/Opening-".strtolower($closeIndxx['code'])."p-".$datevalue2.".txt";
...
//$this->saveProcess(1);
	...
	if($totalindexes<=$page)
		...
		{
			$this->saveProcess(1);
			$this->Redirect2("index.php?module=calcindxxopeningtemp&event=index&id=".($page+$limit),"","");	
		}
//$this->Redirect2("index.php?module=notifyforca","","");	
