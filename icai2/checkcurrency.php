		...
		if (! empty ( $cas )) {
			foreach ( $cas as $ca ) {
				...
				if ($security ['curr'] != $ca ['currency']) {
					...
					$chkindexstartdate = selectrow ( array (
							'name',
							'code' 
					), 'tbl_indxx', array (
							'dateStart' => date ( "Y-m-d" ),
							'id' => $security ['indxx_id'] 
					) );
				...
				$ca_values = selectrow ( array (
						'field_value' 
				), 'tbl_ca_values', array (
						'ca_action_id' => $ca ['action_id'],
						'field_name' => 'CP_DVD_CRNCY' 
				) );
				...
				if ($security ['divcurr'] != $ca_values [0] ['field_value']) {
					...
					$chkindexxstartdate = selectrow ( array (
							'name',
							'code' 
					), 'tbl_indxx', array (
							'dateStart' => date ( "Y-m-d" ),
