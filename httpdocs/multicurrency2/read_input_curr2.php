...
$filecontent=file_get_contents("../files/ca-input/curr1.csv.".date("Ymd",strtotime($date)));
if($filecontent){
...
//	exit;
	...
	if(is_numeric($security[3]))
			...
			{
				mail("dbajpai@indxx.com","Non Numeric Price for Currency file!","Price is not a number for ".$security[0]);
				$i++;	
			}
			}
		}
...
mail("dbajpai@indxx.com","File Read Error!","curr1.csv for today is not available ");
exit;
}
$time = microtime();
$time = explode(' ', $time);
