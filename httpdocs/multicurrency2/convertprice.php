...
echo "totalindexes=" . $totalindexes . "\n";

//currency_hedged='0' and
// status='1' and usersignoff='1' and dbusersignoff='1' and submitted='1' 

$Query = mysql_query("Select  * from tbl_indxx  where  1=1 limit $page,1 ");
if(mysql_num_rows($Query) > 0)
{
	...
	{
		print_r($index); //index - take each index from indxx_table one by one
		$convert_flag=false;
		...
		if($index['currency_hedged']==1)
		{
			...
			{
				echo "HEDGED INDEX DOESNT HAVE SECURITY" . "\n";
				$convert_flag=true;
			}
			else
			{
				echo "HEDGED INDEX HAS SECURITY" . "\n";
				$convert_flag=false;
			}
		}
		else
		{
			echo "NON_HEDGED INDEX" . "\n";
			$convert_flag=true;
		}
		...
		if($convert_flag && 
			...
			$index['submitted'])
		{
			$query="SELECT it.isin,it.ticker,
				(select price from tbl_prices_local_curr pf where pf.isin=it.isin  and pf.date='".$date."') 
						as localprice ,
						(select curr from tbl_prices_local_curr pf where pf.isin=it.isin  and pf.date='".$date."') 
								as local_currency,it.curr as ticker_currency 
								FROM `tbl_indxx_ticker` it where it.indxx_id='".$index['id']."'";
