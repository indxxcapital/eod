...
$finalArray=array();
	...
	foreach($data as $key=> $newcTicker)
	{
		...
		$indxx_value=$this->db->getResult("select tbl_indxx_value.* from tbl_indxx_value where indxx_id='".$newcTicker['indxx_id']."' order by date desc ",false,1);	
		if(!empty($indxx_value))
		{
		$ticker_details=$this->db->getResult("SELECT  it.id,it.name,it.isin,it.ticker,curr,divcurr,curr,sedol,cusip,countryname,(select price from tbl_final_price fp where fp.isin=it.isin  and fp.date='".$indxx_value['date']."' and fp.indxx_id='".$newcTicker['indxx_id']."') as calcprice,(select localprice from tbl_final_price fp where fp.isin=it.isin  and fp.date='".$indxx_value['date']."' and fp.indxx_id='".$newcTicker['indxx_id']."') as localprice,(select share from tbl_share sh where sh.isin=it.isin  and sh.indxx_id='".$newcTicker['indxx_id']."') as calcshare FROM `tbl_indxx_ticker` it where it.indxx_id='".$newcTicker['indxx_id']."' and ticker='".$newcTicker['identifier']."'",false,1);	
	...
	$deleteShareQuery="delete from tbl_share where indxx_id='".$request['indxx_id']."' and isin='".$request['old_ticker']['isin']."' ";
		...
		$this->saveProcess();
	//$this->Redirect("index.php?module=calcspinstockaddtemp","","");	
	
		$this->Redirect2("index.php?module=calccadptemp","","");	
	}
}